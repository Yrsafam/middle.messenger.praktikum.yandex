export const chats = [
  {
    name: "Андрей",
    message: "Изображение",
    lastTime: "10:49",
    count: 2,
    isMe: false,
  },
  {
    name: "Киноклуб",
    message: "стикер",
    lastTime: "12:00",
    count: 0,
    isMe: true,
  },
  {
    name: "Илья",
    message:
      // eslint-disable-next-line max-len
      "Друзья, у меня для вас особенный выпуск новостей! Друзья, у меня для вас особенный выпуск новостей!",
    lastTime: "12:00",
    count: 4,
    isMe: false,
  },
  {
    name: "Вадим",
    message: "Круто!",
    lastTime: "Пт",
    count: 4,
    isMe: false,
  },
  {
    name: "тет-а-теты",
    message: "И Human Interface Guidelines и Material Design рекомендуют...",
    lastTime: "Ср",
    count: 0,
    isMe: false,
  },
  {
    name: "1, 2, 3",
    message: "Миллионы россиян ежедневно проводят десятки часов свое...",
    lastTime: "Пн",
    count: 0,
    isMe: false,
  },
  {
    name: "Design Destroyer",
    message: "В 2008 году художник Jon Rafman начал собирать...",
    lastTime: "Пн",
    count: 0,
    isMe: false,
  },
  {
    name: "Day.",
    message: "Так увлёкся работой по курсу, что совсем забыл его анонсир...",
    lastTime: "1 Мая 2020",
    count: 0,
    isMe: false,
  },
  {
    name: "Стас Рогозин",
    message: "Можно или сегодня или завтра вечером.",
    lastTime: "12 Апр 2020",
    count: 0,
    isMe: false,
  },
  {
    name: "Андрей",
    message: "Изображение",
    lastTime: "10:49",
    count: 2,
    isMe: false,
  },
  {
    name: "Киноклуб",
    message: "стикер",
    lastTime: "12:00",
    count: 0,
    isMe: true,
  },
  {
    name: "Илья",
    message:
      // eslint-disable-next-line max-len
      "Друзья, у меня для вас особенный выпуск новостей! Друзья, у меня для вас особенный выпуск новостей!",
    lastTime: "12:00",
    count: 4,
    isMe: false,
  },
  {
    name: "Вадим",
    message: "Круто!",
    lastTime: "Пт",
    count: 4,
    isMe: false,
  },
  {
    name: "тет-а-теты",
    message: "И Human Interface Guidelines и Material Design рекомендуют...",
    lastTime: "Ср",
    count: 0,
    isMe: false,
  },
  {
    name: "1, 2, 3",
    message: "Миллионы россиян ежедневно проводят десятки часов свое...",
    lastTime: "Пн",
    count: 0,
    isMe: false,
  },
  {
    name: "Design Destroyer",
    message: "В 2008 году художник Jon Rafman начал собирать...",
    lastTime: "Пн",
    count: 0,
    isMe: false,
  },
  {
    name: "Day.",
    message: "Так увлёкся работой по курсу, что совсем забыл его анонсир...",
    lastTime: "1 Мая 2020",
    count: 0,
    isMe: false,
  },
  {
    name: "Стас Рогозин",
    message: "Можно или сегодня или завтра вечером.",
    lastTime: "12 Апр 2020",
    count: 0,
    isMe: false,
  },
];

export const messages = [
  {
    message:
      // eslint-disable-next-line max-len
      "Привет! Смотри, тут всплыл интересный кусок лунной космической истории — НАСА в какой-то момент попросила Хассельблад адаптировать модель SWC для полетов на Луну. Сейчас мы все знаем что астронавты летали с моделью 500 EL — и к слову говоря, все тушки этих камер все еще находятся на поверхности Луны, так как астронавты с собой забрали только кассеты с пленкой.\n" +
      // eslint-disable-next-line max-len
      "Хассельблав итоге адаптировал SWC для космоса, но что-то пошло не так и на ракету они так никогда и не попали. Всего их было произведено 25 штук, одну из них недавно продали на аукционе за 45000 евро.",
    isMe: false,
    time: "11:56",
    photo: "",
  },
  {
    message: "",
    isMe: false,
    time: "11:56",
    photo: "/images/message-photo.jpg",
  },
  {
    message: "Круто!",
    isMe: true,
    time: "12:00",
    photo: "",
  },
];
